"""Model #6 — Grounded description generator.

Calls Groq (Llama 3.1 8B) with a category-matched few-shot prompt built
from Model #1's structured fields and Model #2's visual_wear_probability.
Falls back to a deterministic template on any failure so the main pipeline
is never blocked.

Skips the API call and returns the template when VLM_BACKEND=stub (test
...
_SYSTEM = (
    "You write listing descriptions for second-hand clothing platforms. "
    "Use only the facts you are given — never invent details not in the structured data. "
    "If a field is marked unreliable, do not mention it. "
    "Write in English. Be personal, direct, and honest — write as the seller talking to the buyer. "
    "Mention specific details like wear, fit, or why it stands out. 2–4 sentences. No bullet points."
)

# Category-matched few-shot examples. Each entry is a list of (field_summary, description) pairs.
# Written from sold Vinted listings — descriptions that resulted in a sale.
...
def _build_prompt(fields: dict, platform: str, visual_wear: float, field_review: dict) -> str:
    ...
    example_block = "\n\n".join(
        f"[Example {i + 1}]\n{ex_f}\nDescription: {ex_d}"
        for i, (ex_f, ex_d) in enumerate(examples)
    )
    return (
        f"{example_block}\n\n"
        f"[Write a description for]\n"
...
def _template_fallback(fields: dict, visual_wear: float, field_review: dict) -> str:
    unreliable = set(field_review.get("needs_review", []))
...
async def generate_description(
    ...
    """Generate a polished English listing description via Groq.

    Returns the template fallback if the API key is missing, the VLM backend
    is set to stub (test mode), or the API call fails. Never raises.
    """
    field_review = field_review or {}

    if os.environ.get("VLM_BACKEND") == "stub":
        return _template_fallback(fields, visual_wear, field_review)

    api_key = os.environ.get("GROQ_API_KEY", "")
    if not api_key:
        logger.warning("GROQ_API_KEY not set — using template fallback for description")
        return _template_fallback(fields, visual_wear, field_review)
    ...
    try:
        async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
            ...
            content = content.removeprefix("Description:").strip()
            if content:
                return content
            logger.warning("Groq returned empty content — using template fallback")
    except Exception as exc:
        logger.warning("Groq description call failed (%s) — using template fallback", exc)
